...
#define UI_DISPLAY_RS_PIN		28		// PINK.1, 88, D_RS
#define UI_DISPLAY_RW_PIN		-1
#define UI_DISPLAY_ENABLE_PIN	        29		// PINK.3, 86, D_E
...
#define UI_DISPLAY_D4_PIN		10		// PINF.5, 92, D_D4
#define UI_DISPLAY_D5_PIN		11		// PINK.2, 87, D_D5
#define UI_DISPLAY_D6_PIN		16		// PINL.5, 40, D_D6
#define UI_DISPLAY_D7_PIN		17		// PINK.4, 85, D_D7
...
#define UI_KEYS_ANALOG(num,action_) if ( num == analogkey) action=action_; 
...
// dark magic with chinese keypad start here
  ...
  pinMode(33,INPUT);
  int analogstate =0;
  analogstate=analogRead(33);
  analogstate=analogRead(33);
//  analogkey = 0 ;
  for (byte i=0;i<buttons_per_column;i++) {
    int diff=abs(values[i]-analogstate);
    if (diff<=3) {
      analogkey = i+1;
      break;
    } 
...
// end of dark magic
  ...
  UI_KEYS_ANALOG(1,UI_ACTION_BACK); // left button
  UI_KEYS_ANALOG(3,UI_ACTION_MENU_UP); // right button
  UI_KEYS_ANALOG(4,UI_ACTION_OK);  //Select button
